...
import { SHOP_DERIVED_COLUMNS, SHOP_DERIVED_INDEXES, buildShopDerivedAlter } from '../dist/shophunter/sh.shop-derived.js';
...
const colSet = new Set(haveCols.map((r) => r.n));
const idxSet = new Set(haveIdx.map((r) => r.n));
const missingCols = SHOP_DERIVED_COLUMNS.map((x) => x.name).filter((n) => !colSet.has(n));
...
if (!sql) {
  console.log(`sh_shop đã có đủ ${SHOP_DERIVED_COLUMNS.length} cột dẫn xuất + ${SHOP_DERIVED_INDEXES.length} index — không cần làm gì.`);
  await c.end();
  process.exit(0);
}
