...
- [x] **S2+S3 — Báo cáo:** mở `sh/report`+top-shops/top-products (cap top-list 5)+buckets/order-buckets/shop-orders/order-products cho user; FE CTA khi top-list capped. GIỮ staff-only POST `analyze-now`+`reconcile-shop-revenue` (=403 với user). Smoke: top-shops byRevenue=5 capped.
