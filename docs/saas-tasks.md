...
- [x] **Track Shopify** (2026-09-11, commits `2d2d099`, `4d6356c`, `067b59a`): Quét domain lẻ (`mode: single`) & quét hàng loạt (`mode: bulk` paste/import file txt, csv, xlsx). Tự động phát hiện cửa hàng Shopify, lấy và đồng bộ chuỗi doanh thu ngày (`revenueChart` → `sh_shop_revenue_daily`). Lưu bền vững vào DB Local DB (`sh_shop`) với `shop_url = domain` (đảm bảo tìm kiếm ra ngay tại `https://dpboss.pet/localdb/shops`). Hiển thị bảng Lịch sử 10 cột có đủ DT Ngày, DT Tuần, DT Tháng, Ads/SKU, Quốc gia, Tiền tệ, nút Traffic và Xem chi tiết ▸; hỗ trợ xuất CSV. Bật `force-dynamic` toàn bộ Next.js app triệt tiêu cache chunk cũ.
- [ ] **Track (theo dõi shop nâng cao, per-user notification/alert)**: tính năng theo dõi biến động định kỳ theo tài khoản user (để sau).
...
- [x] **Phần 2 affiliate — rút trích nội quy/luật.** XONG 2026-08-13 (`07e1772`…`4d12c8d`). Thực tế **30%** độ phủ chứ không phải 65% như khảo sát — khảo sát chấm trên text cả trang gồm menu/footer nên là cận TRÊN, đúng như doc khảo sát đã ghi. Bảng `aff_terms`, job nền `affterms` (tab Cài đặt), tóm tắt ghi vào cột Note. Chi tiết: [handoff-2026-08-13-affiliate-noi-quy.md](./handoff-2026-08-13-affiliate-noi-quy.md).
- [ ] **Nội quy có nên ĐÈ Note cũ không?** Hiện chỉ ghi khi Note trống → 22.837/36.241 dòng đã có blurb của mạng (do nút "Điền hoa hồng") sẽ không nhận nội quy vào Note, vẫn xem được qua chip 📋 ở bảng. Nội quy giá trị hơn blurb — cần anh quyết, đổi một dòng điều kiện trong `saveTerms` (`afflib.mysql.ts`).
