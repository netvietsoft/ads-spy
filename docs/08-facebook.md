...
- `GET /api/fb/search?q=<kw>&country=<VN>` → scrape + **lưu DB** → `{ searchId, query, country, count, ads[] }`.
- `GET /api/fb/history` → 20 lượt gần nhất. `GET /api/fb/search/:id` → đọc lại từ DB (KHÔNG chạy Chromium).
- `search` nhận `status` = all|active|inactive (`active_status`). Ads TM VN đã ngừng: Meta không lưu (chỉ political + EU).
- `GET /api/fb/report?country=&range=` → **bảng xếp hạng chi tiêu** (scrape `/ads/library/report/`): mỗi dòng lấy từ
  `innerText` của link `view_all_page_id` → `[tên trang, tuyên bố, chi tiêu ₫, số ads]` + page_id. Bấm dòng → xem ads page đó.
- DB: model `FbSearch` + `FbAd` (mảng platforms/images/videos lưu JSON string). Xem [05](05-du-lieu-va-db.md).
- Web: click thẻ → `FbModal` (carousel ảnh + video + nút tải). Có lịch sử FB + banner "đã lưu". Theme sáng/tối (nút ở header).
